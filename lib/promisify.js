import { toDBusError } from './errors.js';
...
// `Error.captureStackTrace` is a V8 extension. It is present on Node and Deno
// (both V8) and Bun, but feature-detect so a runtime without it still gets a
// stack, just one captured from a fresh Error instead.
...
function maybePromise(callback, run) {
  ...
  const callSite = {};
  captureStackTrace(callSite, maybePromise);
